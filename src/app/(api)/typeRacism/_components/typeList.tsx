...
const TypeRacismList = () => {
  ...
  if (loading) {
    return (
      <section className="my-8 p-6 bg-amber-200 shadow-md rounded-lg">
      ...
      <section className="my-8 p-6 bg-amber-200 shadow-md rounded-lg">
        <h2 className="text-2xl font-bold mb-4">Tipos de Racismo Cadastrados</h2>
         <ul className="space-y-2">
          {dados.map((item) => (
            <li key={item.id} className="p-2 bg-amber-300 rounded">
              {item.descricao}
            </li>
          ))}
          </ul>
        <button
          onClick={list}
          className="mt-4 px-4 py-2 bg-lime-800 text-white rounded hover:bg-lime-900"
        >Mostrar Lista
        </button>
      </section>
    );
